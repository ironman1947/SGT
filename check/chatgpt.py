...
def extract_features(landmarks, frame_shape):
    ...
    features = np.array([[landmark.x * width, landmark.y * height] for landmark in landmarks.landmark]).flatten()
    padded_features = np.pad(features, (0, 63 - features.size), 'constant') if features.size < 63 else features[:63]
    return padded_features

# Define the labels (ensure this matches your training data)
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            processed_frame = extract_features(hand_landmarks, frame.shape)
            processed_frame = processed_frame.reshape(1, 63)  # Reshape for model input

            pred = model.predict(processed_frame)
            print(f"Prediction probabilities: {pred}")  # Print all probabilities for debugging
